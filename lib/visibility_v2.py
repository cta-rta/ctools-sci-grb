...
class Visibility:
    ...
    def get_nighttime(self, twilight=-18, digits=8, num=10):
        ...
        windows = {'start': [], 'stop': []}
        current = None
        #f = interp1d(self.sun_altaz.alt.value, self.vis_points.value, kind=3)
        for idx, t in enumerate(self.vis_points.value):
            ...
            if idx == 0 and current is True:
                ...
                if idx > num and idx < len(self.vis_points)-num:
                    x = np.array(self.sun_altaz[idx-num:idx+num].alt.value)
                    y = np.array(self.vis_points[idx-num:idx+num].value)
                elif idx <= num:
                    x = np.array(self.sun_altaz[0:idx+num].alt.value)
                    y = np.array(self.vis_points[0:idx+num].value )
                elif idx >= len(self.vis_points)-num:
                    x = np.array(self.sun_altaz[idx-num:len(self.vis_points)].alt.value)
                    y = np.array(self.vis_points[idx-num:len(self.vis_points)].value)
        ...
        for idx, t in enumerate(self.vis_points.value):
            ...
            if idx == 0 and current is True:
                ...
                if sun_cond[idx] != sun_cond[idx - 1]:
                    ...
                    use = 'moon'                
                # interpolate
                f = interp1d(np.array(x), np.array(y), kind=3)
                if previous is False and current is True:
                    if use == 'sun':
                        windows['start'].append(f(twilight))
                    elif use == 'moon':
    ...
    def associate_irf_zenith_angle(self, thresholds=(10, 25, 55), zenith_angles=(60, 40, 20), digits=8, num=10):
        """
        ...
        #f = interp1d(self.altaz.alt.value, self.vis_points.value, kind=3)
        for idx, alt in enumerate(self.altaz.alt.value):
            ...
            if idx > num and idx < len(self.vis_points)-num:
                x = np.array(self.altaz[idx-num:idx+num].alt.value)
                y = np.array(self.vis_points[idx-num:idx+num].value)
            elif idx <= num:
                x = np.array(self.altaz[0:idx+num].alt.value)
                y = np.array(self.vis_points[0:idx+num].value )
            elif idx >= len(self.vis_points)-num:
                x = np.array(self.altaz[idx-num:len(self.vis_points)].alt.value)
                y = np.array(self.vis_points[idx-num:len(self.vis_points)].value)
